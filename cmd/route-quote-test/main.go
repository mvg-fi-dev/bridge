...
func main() {
	...
	// Route bot public key (base64url) can be supplied directly to avoid depending on Mixin API.
	botPK := os.Getenv("ROUTE_BOT_PUBLIC_KEY")
	if botPK == "" {
		// Otherwise fetch route bot session public key via Mixin API
		ksPath := os.Getenv("MIXIN_KEYSTORE_PATH")
		if ksPath == "" {
			log.Fatal("set ROUTE_BOT_PUBLIC_KEY or MIXIN_KEYSTORE_PATH to fetch route bot session")
		}
