...
func (r *OrdersRepo) ListWithdrawing(ctx context.Context, limit int) ([]*models.Order, error) {
	if limit <= 0 {
		limit = 50
	}
...
FROM orders
WHERE status = ? AND (withdraw_txid IS NULL OR withdraw_txid = '')
ORDER BY updated_at ASC
LIMIT ?
`, string(models.StatusWithdrawing), limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
